...
def test_phase11_judge_verdicts_migration_round_trip() -> None:
    """Round-trip migration through Phase 6 head (024).

    Verifies:
    - upgrade head reaches 024 without error
    - `alembic check` reports no drift between head and ORM metadata
    - downgrade to 023 succeeds (back to Phase 5 head)
    - upgrade back to head succeeds (re-apply 024)
