...
def test_phase12_pending_approvals_migration_round_trip() -> None:
    """Round-trip migration through Phase 7 head (025).

    Verifies:
    - upgrade head reaches 025 without error
    - `alembic check` reports no drift between head and ORM metadata
    - downgrade to 024 succeeds (back to Phase 6 head)
    - upgrade back to head succeeds (re-apply 025)
    - downgrade to base succeeds (full teardown)
    """
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "alembic_phase12.db"
        _run_alembic(["upgrade", "head"], db_path)
        current = _run_alembic(["current"], db_path)
        assert "025" in current.stdout + current.stderr, (
            f"expected head=025; got stdout={current.stdout!r} "
        ...
        _run_alembic(["downgrade", "024"], db_path)
        _run_alembic(["upgrade", "head"], db_path)
