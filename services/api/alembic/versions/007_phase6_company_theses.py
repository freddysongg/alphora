...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["company_entity_id"], ["entities.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.UniqueConstraint(
            "run_id", "company_entity_id", name="uq_company_theses_run_company"
        ),
        sa.CheckConstraint(
            "direction IN ('overweight', 'underweight', 'neutral')",
            name="ck_company_theses_direction",
        ),
        sa.CheckConstraint(
            "verifier_status IN ('verified', 'quote_unverified')",
            name="ck_company_theses_verifier_status",
        ),
        sa.CheckConstraint(
            "judge_status IN ('not_run', 'passed', 'flagged')",
            name="ck_company_theses_judge_status",
        ),
    )
    op.create_index("ix_company_theses_run_id", "company_theses", ["run_id"])


def downgrade() -> None:
    op.drop_index("ix_company_theses_run_id", table_name="company_theses")
    op.drop_table("company_theses")
