"""Per-run extraction orchestration for company fan-out.
...
@dataclass(frozen=True)
class CompanyExtractionOutcome:
    results: list[ExtractionResult]
    failures: list[ExtractionFailure]


async def extract_company_chunks(
    *,
    ...
    """
    if not chunks:
        return CompanyExtractionOutcome(results=[], failures=[])
        ...
        async with semaphore:
            try:
                ...
                emit_run_event(
                    ...
                    message=(
                        f"company extraction failed for chunk {chunk.chunk_id}: {exc}"
                    ),
                    data={
                        "event": "company_extraction_failure",
    ...
    return CompanyExtractionOutcome(results=results, failures=failures)
